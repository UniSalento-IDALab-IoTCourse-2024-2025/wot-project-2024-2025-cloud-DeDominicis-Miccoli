"""
PIEZO Anomaly Detection Model Trainer
...
class PIEZOModelTrainer:
    """
    Trains PIEZO anomaly detection model using autoencoder
    """
    ...
    def train(self, train_data, val_data):
        ...
        print(f"\n[PIEZOTrainer] Starting training...")
    ...
    def calculate_threshold(self, train_data):
        ...
        print(f"\n[PIEZOTrainer] Calculating threshold...")
    ...
    def plot_examples(self, data, n_examples=6):
        ...
        for i in range(n_examples):
            ...
            axes[i].set_title(f"PIEZO Sample {idx}", fontsize=11)
    ...
    def plot_reconstruction(self, data, n_examples=4):
        ...
        for i, idx in enumerate(indices):
            ...
            axes[i].plot(data[idx], 'b', label='Original PIEZO', linewidth=1.5)
            ...
            axes[i].set_title(f'PIEZO Sample - MSE: {mse:.6f}', fontsize=12)
...
def train_piezo_model(training_id, session_ids, model_config, training_manager):
    """
    Main training function for PIEZO model
    ...
    try:
        print(f"\n{'='*60}")
        print(f"STARTING PIEZO MODEL TRAINING")
        print(f"Training ID: {training_id}")
        print(f"{'='*60}\n")
        
        trainer = PIEZOModelTrainer(training_id, training_manager)
        
        # STEP 1: Load data
        print("STEP 1: Loading PIEZO data from sessions...")
        loader = SessionDataLoader()
        raw_signal, sessions_metadata = loader.load_piezo_data(session_ids)
...
def package_and_upload_to_raspberry(model_folder, model_type='piezo'):
    """
    Package trained model and automatically upload to Raspberry Pi
    
    Args:
        model_folder: Path to model folder (e.g., models/piezo/piezo_model_v1_20250116)
        model_type: 'piezo' or 'piezo'
  ...
  "auto_upload": true
    ...
    try:
        # Generate proper model name: piezo_model_v1_20251216
